import type { PermissionV1 } from "@agnes-ai/core/v1/permission"
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@agnes-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run agnescode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running agnescode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AGNESCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to AGNESCODE_SERVER_USERNAME or 'agnescode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createOpencodeClient({
        baseUrl: "http://agnescode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
