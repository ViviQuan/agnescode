...
import type { ProviderV2 } from "@agnes-ai/core/provider"
import { isContextOverflow } from "@agnes-ai/llm"
...
function message(providerID: ProviderV2.ID, e: APICallError) {
  return iife(() => {
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `agnescode auth login <your provider URL>` to re-authenticate."
      }
      if (e.statusCode === 403) {
