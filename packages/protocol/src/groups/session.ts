...
import { PromptInput } from "@agnes-ai/schema/prompt-input"
import { Session } from "@agnes-ai/schema/session"
import { Project } from "@agnes-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@agnes-ai/schema/schema"
...
import { SessionEvent } from "@agnes-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this AgnesCode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this AgnesCode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
