...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@agnes-ai/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.AGNESCODE_ZED_DB
  process.env.AGNESCODE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.AGNESCODE_ZED_DB
    else process.env.AGNESCODE_ZED_DB = previous
    home.mockRestore()
  }
})
