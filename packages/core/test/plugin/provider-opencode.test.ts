...
import { Catalog } from "@agnes-ai/core/catalog"
import { Credential } from "@agnes-ai/core/credential"
import { EventV2 } from "@agnes-ai/core/event"
import { Integration } from "@agnes-ai/core/integration"
import { ModelV2 } from "@agnes-ai/core/model"
import { PluginV2 } from "@agnes-ai/core/plugin"
import { PluginHost } from "@agnes-ai/core/plugin/host"
import { OpencodePlugin } from "@agnes-ai/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
      ...
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("opencode")))?.methods).toEqual([
        {
          ...
          type: "oauth",
          label: "AgnesCode Console account",
        },
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
      ...
      expect(attempt.url).toBe("https://console.agnescode.ai/console/device?user_code=user&client_id=opencode-cli")
    }),
  )
  ...
  it.live("loads providers and models from the connected AgnesCode server", () =>
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ AGNESCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.agnescode),
        ...
        expect(required(yield* catalog.model.get(ProviderV2.ID.agnescode, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ AGNESCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.agnescode)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.agnescode, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("uses AGNESCODE_API_KEY as credentials", () =>
    withEnv({ AGNESCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.agnescode)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.agnescode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ AGNESCODE_API_KEY: undefined, CUSTOM_AGNESCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_AGNESCODE_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ AGNESCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.agnescode)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.agnescode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("prefers gpt-5-nano as the opencode small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.agnescode
