...
import type { AssistantMessage, Message, OpencodeClient, Session, SessionMessageResponse } from "@agnes-ai/sdk/v2"
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `agnescode auth login` in the terminal",
      name: "Login with agnescode",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, agnes provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const agnesProvider = providers[ProviderV2.ID.make("agnes")]
  const agnesModel = agnesProvider ? Provider.sort(Object.values(agnesProvider.models))[0] : undefined
  if (agnesProvider && agnesModel) return { providerID: agnesProvider.id, modelID: agnesModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "AgnesCode prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "AgnesCode service failure", service })
}
