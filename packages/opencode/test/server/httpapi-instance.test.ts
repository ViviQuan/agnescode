...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.AGNESCODE_WORKSPACE_ID
      Flag.AGNESCODE_WORKSPACE_ID = WorkspaceV2.ID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.AGNESCODE_WORKSPACE_ID = originalWorkspaceID
        }),
      )
