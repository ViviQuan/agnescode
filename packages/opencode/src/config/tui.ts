...
import { isRecord } from "@agnes-ai/tui/util/record"
...
import { TuiKeybind } from "@agnes-ai/tui/config/keybind"
...
import { TuiConfig } from "@agnes-ai/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old agnescode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.agnescode`
  // folders between cwd and home, and AGNESCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AGNESCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AGNESCODE_TUI_CONFIG override, if set.
  if (Flag.AGNESCODE_TUI_CONFIG) {
    const configFile = Flag.AGNESCODE_TUI_CONFIG
  ...
  // 4. `.agnescode` directories (and AGNESCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".agnescode") || dir === Flag.AGNESCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".agnescode") && dir !== Flag.AGNESCODE_CONFIG_DIR) continue
