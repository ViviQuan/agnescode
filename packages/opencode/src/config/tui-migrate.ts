...
const TUI_SCHEMA_URL = "https://agnescode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from agnescode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const cfgFiles = await agnescodeFiles(input)
...
async function agnescodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "agnescode"),
    ...(await Filesystem.findUp(["agnescode.json", "agnescode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "agnescode"))
  }
  if (Flag.AGNESCODE_CONFIG) files.push(Flag.AGNESCODE_CONFIG)
