...
it.instance("plan agent denies edits except .agnescode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".agnescode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".agnescode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AGNESCODE_TEST_HOME
      process.env.AGNESCODE_TEST_HOME = test.directory
