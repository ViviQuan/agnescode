...
import { McpEvent } from "@agnes-ai/schema/mcp-event"
...
function createClient(directory: string) {
  const client = new Client({ name: "agnescode", version: InstallationVersion }, CLIENT_OPTIONS)
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: agnescode mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
