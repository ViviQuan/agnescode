...
import { RelativePath } from "@agnes-ai/core/schema"
...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".agnescode")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".agnescode", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".agnescode/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".agnescode/config"))
