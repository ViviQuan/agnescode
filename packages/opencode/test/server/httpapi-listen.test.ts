...
const original = {
  ...
  envPassword: process.env.AGNESCODE_SERVER_PASSWORD,
  envUsername: process.env.AGNESCODE_SERVER_USERNAME,
}
const auth = { username: "agnescode", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.AGNESCODE_SERVER_PASSWORD
  else process.env.AGNESCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.AGNESCODE_SERVER_USERNAME
  else process.env.AGNESCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.AGNESCODE_SERVER_PASSWORD = auth.password
  Flag.AGNESCODE_SERVER_USERNAME = auth.username
  process.env.AGNESCODE_SERVER_PASSWORD = auth.password
  process.env.AGNESCODE_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.AGNESCODE_SERVER_USERNAME = auth.username
  delete process.env.AGNESCODE_SERVER_PASSWORD
  process.env.AGNESCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    await using tmp = await tmpdir({
      init: async (directory) => {
        ...
        await Bun.write(
          path.join(directory, "agnescode.json"),
          JSON.stringify({ formatter: false, lsp: false, plugin: [pathToFileURL(plugin).href] }),
        )
        return { initialized, completed }
      },
    })
    const previous = process.env.AGNESCODE_DISABLE_DEFAULT_PLUGINS
    process.env.AGNESCODE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.AGNESCODE_DISABLE_DEFAULT_PLUGINS
      else process.env.AGNESCODE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
