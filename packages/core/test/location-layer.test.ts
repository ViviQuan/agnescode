...
import { define } from "@agnes-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@agnes-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "agnescode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "agnescode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
