...
import { FileSystemWatcher } from "@agnes-ai/schema/filesystem-watcher"
...
declare const AGNESCODE_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof AGNESCODE_LIBC === "undefined" ? undefined : AGNESCODE_LIBC
...
export const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.AGNESCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (yield* Flag.AGNESCODE_EXPERIMENTAL_FILEWATCHER) {
