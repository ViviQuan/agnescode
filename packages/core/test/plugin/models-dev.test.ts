...
import { ModelsDevPlugin } from "@agnes-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.AGNESCODE_MODELS_PATH,
          disabled: Flag.AGNESCODE_DISABLE_MODELS_FETCH,
        }
        Flag.AGNESCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.AGNESCODE_MODELS_PATH = previous.path
          Flag.AGNESCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
