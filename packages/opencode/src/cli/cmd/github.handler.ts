...
const AGENT_USERNAME = "agnescode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/agnescode.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://agnescode.ai/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        async function getInstallation() {
          return await fetch(`https://api.agnescode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  agnescode:
    if: |
      ...
      contains(github.event.comment.body, ' /agnescode') ||
      startsWith(github.event.comment.body, '/agnescode')
    ...
    steps:
      ...
      - name: Run agnescode
        uses: anomalyco/agnescode/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.agnescode.ai" : "https://agnescode.ai"
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.agnescode.ai"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/agnescode,/oc")
        .split(",")
