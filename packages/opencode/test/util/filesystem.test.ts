...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "agnescode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "agnescode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agnescode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agnescode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["agnescode.json", "agnescode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "agnescode.json"),
        path.join(project, "agnescode.jsonc"),
      ])
    })
  })
