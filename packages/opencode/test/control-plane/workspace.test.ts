...
import { EventSequenceTable } from "@agnes-ai/core/event/sql"
...
import { WorkspaceV2 } from "@agnes-ai/core/workspace"
import { WorkspaceTable } from "@agnes-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  AGNESCODE_AUTH_CONTENT: process.env.AGNESCODE_AUTH_CONTENT,
  AGNESCODE_EXPERIMENTAL_WORKSPACES: process.env.AGNESCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.AGNESCODE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.AGNESCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.AGNESCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.AGNESCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.AGNESCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
