...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@agnes-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@agnes-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@agnes-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@agnes-ai/tui/context/theme"
import { Global } from "@agnes-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@agnes-ai/core/util/flock"
import { Flag } from "@agnes-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@agnes-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@agnes-ai/core/v1/config/plugin"
import { createCommandShim } from "@agnes-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@agnes-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".agnescode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".agnescode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".agnescode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AGNESCODE_PURE ? [] : pluginOrigins
    if (Flag.AGNESCODE_PURE && pluginOrigins.length) {
    }
