...
import type { IntegrationOAuthMethodRegistration } from "@agnes-ai/plugin/v2/effect/integration"
import { define } from "@agnes-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@agnes-ai/sdk/v2/types"
...
const defaultServer = "https://console.agnescode.ai"
const clientID = "agnes-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load AgnesCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      const item = catalog.provider.get(ProviderV2.ID.agnescode)
      if (!item) return
      const hasKey = Boolean(process.env.AGNESCODE_API_KEY || connected || item.provider.request.body.apiKey)
