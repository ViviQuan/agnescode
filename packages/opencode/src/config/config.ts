...
import { serviceUse } from "@agnes-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@agnes-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@agnes-ai/core/v1/config/console-state"
import { FSUtil } from "@agnes-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@agnes-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@agnes-ai/core/v1/config/config"
import { RemoteAuthError } from "@agnes-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@agnes-ai/core/v1/config/permission"
...
function globalConfigFile() {
  const candidates = ["agnescode.jsonc", "agnescode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://agnescode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.AGNESCODE_CONFIG && !Flag.AGNESCODE_CONFIG_DIR && !Flag.AGNESCODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://agnescode.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "agnescode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "agnescode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://agnescode.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "AGNESCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://agnescode.ai/config.json"
        ...
        if (Flag.AGNESCODE_CONFIG) {
          yield* merge(Flag.AGNESCODE_CONFIG, yield* loadFile(Flag.AGNESCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.AGNESCODE_CONFIG })
        }

        if (!Flag.AGNESCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("agnescode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.AGNESCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from AGNESCODE_CONFIG_DIR", { path: Flag.AGNESCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".agnescode") || dir === Flag.AGNESCODE_CONFIG_DIR) {
            for (const file of ["agnescode.json", "agnescode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.agnescode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.AGNESCODE_CONFIG_CONTENT) {
          const source = "AGNESCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.AGNESCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from AGNESCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["AGNESCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("AGNESCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.AGNESCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.AGNESCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("AGNESCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.AGNESCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.AGNESCODE_DISABLE_PRUNE) {
