...
- Author package entrypoint is `@agnes-ai/plugin/tui`.
...
```json
{
  "$schema": "https://agnescode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@agnes-ai/plugin/tui`.
- `@agnes-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@agnes-ai/plugin/tui"
...
- If `engines.agnescode` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.agnescode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.agnescode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.agnescode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.agnescode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@agnes-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@agnes-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@agnes-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.agnescode/themes` area near the plugin config source.
...
- Local smoke plugin: `.agnescode/plugins/tui-smoke.tsx`
- Local vim plugin: `.agnescode/plugins/tui-vim.tsx`
- Local smoke config: `.agnescode/tui.json`
- Local smoke theme: `.agnescode/plugins/smoke-theme.json`
